...
DATA_PATH_DB <- "test_data"#'kbda_lab.dbo.tmp_AKB_kunden_mitTRX'

INPUTS_PATH <- "inst" #paste(getwd(), "inputs", sep = "/")
RASTERSTACK_PATH <- "inst/rasterstack_ch" #paste(getwd(), "inputs","rasterstack_ch", sep = "/")
OUT_PATH <- "tests/geo_features.csv"
...
# Set up SQLite DB with content of inst/test_data_ptc_TRX_MIT_KUNDE_20210525.xlsx
odbc_conn <- DBI::dbConnect(RSQLite::SQLite(), "tests/test.db")
test_dat <- readxl::read_excel("tests/test_data_ptc_TRX_MIT_KUNDE_20210525.xlsx")
DBI::dbWriteTable(odbc_conn, DATA_PATH_DB, test_dat)
...
df_features$date <- as.POSIXct(df[,TIME_COLUMN], origin = "1970-01-01")
...
for (bt in c(businesstypes)){
  ...
  if(bt=="groceries" | bt=="restaurant" | bt=="gas" | bt=="shopping"){
    ...
    dt_bt_sub <- data.table::data.table(df_bt_sub)[, .(
      xcount = .N,
      xpct = .N,
      q3 = quantile(as.numeric(get(AMOUNT_COLUMN)),probs = 1/3, names = F, na.rm=T)-
           quantile(as.numeric(get(AMOUNT_COLUMN)),probs = 2/3, names = F, na.rm=T),
      q6 = quantile(as.numeric(get(AMOUNT_COLUMN)),probs = 1/3, names = F, na.rm=T)),
      by = CUSTOMER_ID_COLUMN]
    ...
    spending_results[,name_pct] <- spending_results[,get(name_pct)] / spending_results[,total_activity]
  ...
  # Other businesstypes
    ...
    dt_bt_sub <- data.table::data.table(df_bt_sub)[, .(
      xcount = .N,
      xpct = .N,
      xmedian = median(as.numeric(get(AMOUNT_COLUMN)), na.rm=T)),
      by = CUSTOMER_ID_COLUMN]
...
if(grepl("count", fn)|grepl("_pct", fn)){
  spending_results[,fn] <- data.table::nafill(spending_results[,get(fn)], fill=0)}}
...
DBI::dbWriteTable(conn = odbc_conn, OUT_DB, features.out)
